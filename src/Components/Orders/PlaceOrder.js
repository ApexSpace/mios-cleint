...
const PlaceOrder = () => {
    ...
    const calculateShippingcost = (city) => {
        if (parseFloat(weight / 1000) <= 0.5000) {
            ...
            const additionalPrice = shippCat.find(item => item.weight === "greater");
            if (city === "Lahore") {
                const additionalWeight = Math.ceil(weight) - 1;
                const additionalPriceforWeight = additionalWeight * additionalPrice.incity;
                const shppingTotal = Priceforone.incity + additionalPriceforWeight;
                setShipping(shppingTotal);
                setTotal(subTotal + shppingTotal);
            } else {
                const additionalWeight = Math.ceil(weight) - 1;
                const additionalPriceforWeight = additionalWeight * additionalPrice.outcity;
                                ...
                                <tbody>
                                    {CartItems?.cart.map((i, ind) => {
                                        return (<tr key={ind}>
                                            ...
                                            <td>
                                                Rs.
                                                {user.isAdmin === false && user.role === "dropshipper" && user.dropShipperStatus === true ?
                                                    <span className="product-price">
                                                        <span>{i.product.onSale && i.product.dropshipperPrice ? i.product.dropshipperPrice : i.product.dropshipperPrice}</span>
                                                    </span> : null}
                                                {user.isAdmin === false && user.role === 'wholeseller' ?
                                                    <span className="product-price">
                                                        <span>{i.product.onSale && i.product.discountedPriceW ? i.product.discountedPriceW : i.product.wholesalePrice} </span>
                                                    </span> : null}
                                            </td>
                                            <td>{i.quantity}</td>
                                            <td>Rs.{user.isAdmin === false && user.role === "dropshipper" && user.dropShipperStatus === true ?
                                                <span className="product-price">
                                                    <span>{i.product.onSale && i.product.dropshipperPrice ? i.product.dropshipperPrice * i.quantity : i.product.dropshipperPrice * i.quantity} Rs.</span>
                                                </span> : null}
                                                {user.isAdmin === false && user.role === 'wholeseller' ?
                                                    <span className="product-price">
