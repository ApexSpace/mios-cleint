      ...
      <div className="container mt-3">
        {loading ? <Loader /> : <><h2 className="text-center">Product Details</h2>
          <div className="d-flex justify-content-between my-4">
            <div>
              <h5 className="text-center"> Ordered By </h5> {orderProduct?.user?.name}({orderProduct?.user?.role})
            </div>
            ...
            <div>
              ...
              <h5 className="text-center">  Ordered At </h5> {new Date(orderProduct?.date).toLocaleString('en-PK', { timeZone: 'Asia/Karachi' })}
            </div>
          ...
          <table className="table table-striped table-bordered">
            <thead className="thead-dark">
              <tr className="table-dark text-center">
                ...
                {
                  orderProduct?.user?.role == "dropshipper" && <th scope="col">Dropship Price</th>
                }
                <th scope="col">Qty</th>
            ...
            <tbody>
              {orderProduct.products &&
                orderProduct.products.map((el, index) => {
                  return (
                    <OrderProductDetailMap key={index} product={el} otherdata={orderProduct} orderID={id} />
                  )
                })}
            </tbody>
          </table></>}
