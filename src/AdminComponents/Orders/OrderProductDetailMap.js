...
const OrderProductDetailMap = ({ product, orderID, otherdata }) => {
  ...
  const [forupdateDropshipPrice, setForupdateDropshipPrice] = useState(
    el.product.dropshipperPrice
  );
  const mimimumDropshipPrice =
  ...
  const updateDropshipPrice = (id, price) => {
    axios
      .put(`${host}/api/order/dropshippriceupdateadmin/${orderID}`, {
        productID: id,
        dropshipperPrice: price,
      })
      .then((res) => {
          ...
          <div className="d-flex justify-content-between">
            ...
            <div>
              <button
                ...
                onClick={() =>
                  handleDropshipPriceUpdate(el.product._id, dropshipPrice)
                }
              >
                ✓
              </button>
